...
def regen_time(model, base_state, restore_set, ind_heads, prev_heads, V, nu, args, rep, base, device, seed):
    ...
    model.train()
    if args.opt == "adafactor":                                          # state-free optimizer -> fits 1b in ~7G (10G card)
...
def main():
    ...
    ap.add_argument("--opt", choices=["adamw", "adafactor"], default="adamw"); ap.add_argument("--grad_ckpt", action="store_true")
    args = ap.parse_args()
    if args.smoke:
