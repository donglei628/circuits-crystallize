...
def main():
    ...
    ap.add_argument("--init_emb", default=None, help="load this model's embed_in/embed_out as the embedding geometry (e.g. EleutherAI/pythia-160m)")
    ap.add_argument("--freeze_emb", action="store_true", help="freeze the injected embeddings (keep the geometry fixed)")
    ...
    if args.init_emb:                                         # inject a real model's token geometry (barrier-ladder R3)
        ...
        print(f"  injected embeddings from {args.init_emb} (frozen={args.freeze_emb})", flush=True)
    opt = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=args.lr, betas=(0.9, 0.95), weight_decay=0.01)
    print(f"device={device} nu={args.nu} seed={args.seed} {args.layers}L x{args.heads}H d{args.hidden} V{args.vocab} "
